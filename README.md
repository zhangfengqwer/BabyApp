...
0.4.33：iPhone 网页版同步家庭身份选择/记住身份、成员关系管理、宝宝名片编辑、按媒体日期分组发布（JPEG EXIF / MOV-MP4 元数据，无效或未知时间回退手选日期）和详情长按单媒体移除。Android/PWA 减号均改为单层深色半透明圆钮，无外层白圈，保留 48dp / 44px 点击范围和删除确认。Android 编译与 13 项测试、后端 25 项测试、网页日期逻辑 4 项测试通过。手机浏览器和 VoiceOver 真机验收仍需用户检查。
